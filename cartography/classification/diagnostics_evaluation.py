...
def determine_categories_by_fields(fields):
    ...
    for field in fields[:-4]:
        if field == '':
            ...
            # Usually same coarse category.
            example_categories.append(fine_to_coarse[field.split(";")[0]])
        else:
...
def diag_test_modifier(original_diag_tsv, output_tsv):
    ...
    diag_original, diag_headers = read_glue_tsv(
        original_diag_tsv, guid_index=None)
    ...
    with open(output_tsv, "w") as outfile:
        ...
        for i, (key, line) in enumerate(diag_original.items()):
            ...
            example_categories = determine_categories_by_fields(
                fields=in_fields)
            ...
            out_fields = [out_record[field]
                          if field in out_record else "" for field in FIELDS]
    ...
    with open(eval_filename, "w") as outfile:
        for p in predictions:
            ...
            if guid not in diagnostics:
                raise ValueError(
                    f"Could not find predicted GUID: {p['guid']} in Diagnostic NLI test")
        ...
        for cat, total in coarse_gold_labels.items():
            cat_r3 = matthews_corrcoef(
                y_true=coarse_gold_labels[cat], y_pred=coarse_predicted_labels[cat])
            cat_results = (np.array(coarse_gold_labels[cat]) == np.array(
                coarse_predicted_labels[cat]))
        ...
        overall_r3 = matthews_corrcoef(
            y_true=gold_labels, y_pred=predicted_labels)
